...
int 
src_simple_plain(float* data_in, float* data_out, int input_frames, int output_frames, float src_ratio, int converter_type, int channels)
{
	SRC_DATA data;
	...
	// // log info
	// LOGE("ccdd");
	// LOGE("inputFrames: %d, outputFrames: %d, srcRatio: %f, converterType: %d, channel: %d", input_frames, output_frames, src_ratio, converter_type, channels);
	// if(input_frames >= 2){
	// 	LOGE("%f, %f", data_in[0], data_in[1]);
	// }
	// // end log info

	int err = src_simple(&data, converter_type, channels);
	if (err)
		return 0;
	else
